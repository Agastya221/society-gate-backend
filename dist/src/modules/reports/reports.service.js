...
class ReportsService {
    ...
    async getDashboardStats(societyId) {
        ...
        const [totalResidents, totalFlats, totalGuards, todayEntries, pendingEntries, activeGatePasses, openComplaints, activeEmergencies, todayNotices,] = await Promise.all([
            Client_1.prisma.user.count({ where: { societyId, role: 'RESIDENT', isActive: true } }),
            Client_1.prisma.flat.count({ where: { societyId } }),
            Client_1.prisma.user.count({ where: { societyId, role: 'GUARD', isActive: true } }),
            ...
            Client_1.prisma.entry.count({ where: { societyId, status: 'PENDING' } }),
            Client_1.prisma.gatePass.count({ where: { societyId, status: 'APPROVED' } }),
            Client_1.prisma.complaint.count({ where: { societyId, status: { in: ['OPEN', 'IN_PROGRESS'] } } }),
            Client_1.prisma.emergency.count({ where: { societyId, status: 'ACTIVE' } }),
            Client_1.prisma.notice.count({
                where: {
                    societyId,
    ...
    // Entry statistics
    async getEntryStatistics(societyId, days = 7) {
    ...
    // Peak hours analysis
    async getPeakHoursAnalysis(societyId, days = 30) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        const entries = await Client_1.prisma.entry.findMany({
            where: {
    ...
    // Delivery patterns
    async getDeliveryPatterns(societyId, days = 30) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        const deliveries = await Client_1.prisma.entry.findMany({
            where: {
                societyId,
                type: 'DELIVERY',
                checkInTime: { gte: startDate },
    ...
    async getComplaintStatistics(societyId) {
        const complaints = await Client_1.prisma.complaint.findMany({
            where: { societyId },
            select: {
                category: true,
                status: true,
    ...
    async getVisitorFrequencyReport(societyId) {
        const frequentVisitors = await Client_1.prisma.visitorFrequency.findMany({
            where: {
                societyId,
                isFrequent: true,
            },
    ...
    async getSocietyHealthScore(societyId) {
        const [openComplaints, totalComplaints, activeEmergencies, pendingEntries, totalEntries,] = await Promise.all([
            Client_1.prisma.complaint.count({ where: { societyId, status: { in: ['OPEN', 'IN_PROGRESS'] } } }),
            Client_1.prisma.complaint.count({ where: { societyId } }),
            Client_1.prisma.emergency.count({ where: { societyId, status: 'ACTIVE' } }),
            Client_1.prisma.entry.count({ where: { societyId, status: 'PENDING' } }),
            Client_1.prisma.entry.count({ where: { societyId } }),
        ]);
        let score = 100;
